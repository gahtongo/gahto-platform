...
export default function AdminNewsPage() {
  ...
  const handleOptimizeWithAI = async () => {
    ...
    try {
      ...
      const res = await authFetch(`${API_BASE}/api/v1/ai/optimize-content`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleAiExtract = async () => {
    ...
    if (!rawContent.trim()) {
      setErrorText("Please paste some content or a URL first.");
      return;
    }

    try {
      setIsAiExtracting(true);
      setStatusText("AI is extracting news content...");

      const res = await authFetch(`${API_BASE}/api/v1/news/admin/ai-extract`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!res.ok) {
        setErrorText(data.detail || "AI extraction failed.");
        return;
      }
      ...
      setForm((prev) => ({
        ...
        headline: data.card_headlines?.[0] || "",
      }));

      setStatusText("AI extraction complete. Please review before saving.");
      setRawContent("");
    } catch {
      setErrorText("Unable to connect to AI extraction service.");
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
        <div className="mb-6 rounded-2xl bg-violet-50 p-6 border border-violet-100">
          <div className="flex items-center gap-3">
            <Sparkles className="h-5 w-5 text-violet-700" />
            <h3 className="font-bold text-violet-900">AI News Automation</h3>
          </div>
          <p className="mt-2 text-sm text-violet-700 leading-relaxed">
            Paste a raw article URL or raw text below. GAHTO AI will extract, clean, and pre-fill the form professionally.
          </p>
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            <textarea
              value={rawContent}
              onChange={(e) => setRawContent(e.target.value)}
              placeholder="Paste article URL or raw news text here..."
              className="flex-1 rounded-xl border border-violet-200 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-violet-500"
              rows={2}
            />
            <button
              type="button"
              disabled={isAiExtracting || !rawContent.trim()}
              onClick={handleAiExtract}
              className="inline-flex h-fit items-center justify-center gap-2 rounded-xl bg-violet-700 px-6 py-3 font-semibold text-white shadow-md transition hover:bg-violet-800 disabled:opacity-50"
            >
              {isAiExtracting ? (
                <>
